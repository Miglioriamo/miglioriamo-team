...
/**
 * Estrae la traccia audio di un video, pronta per la trascrizione.
 *
 * Attenzione a un limite fisico: l'audio è interlacciato al video dentro il
 * file, quindi ffmpeg deve leggere buona parte del filmato anche volendo solo
 * il sonoro. Per i reel (pochi MB) è istantaneo; su un video pesante no, e per
 * questo chi chiama può fissare un tetto.
 *
 * Ritorna un Buffer mp3 mono a 16 kHz (leggero: circa 4 MB ogni 10 minuti),
 * oppure null se il video non ha parlato o qualcosa non va.
 */
export async function extractAudio(url, { maxSecondi = 480, timeoutMs = 45000 } = {}) {
  const dir = await mkdtemp(path.join(tmpdir(), "studio-audio-"));
  try {
    ...
    const { code } = await run(
      [
        "-y", "-i", url,
        "-t", String(maxSecondi),
        "-vn",              // niente video: si tiene solo il sonoro
        "-ac", "1",         // mono: per il parlato basta e pesa la metà
        "-ar", "16000",     // 16 kHz: lo standard per il riconoscimento vocale
        "-b:a", "32k",
        out,
      ],
      timeoutMs
    ...
    if (code !== 0) return null;
    const dati = await readFile(out);
    // Un file minuscolo vuol dire silenzio o traccia assente.
    return dati.length > 2000 ? dati : null;
  } catch {
    return null;
  } finally {
