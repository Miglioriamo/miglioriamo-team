...
//   1. si scrive SOLO dentro la cartella del cliente: `output/` (i risultati),
//      `contesto-generico/` (il fascicolo) o `memoria/` (cosa è già stato fatto).
//      Mai altrove.
...
import { uploadFile, copyFile, getMetadata, downloadText } from "./dropbox.js";

// "report" resta ammesso per quello già archiviato prima del cambio nome:
// il nostro report ora si chiama "report-profili", perché quello sulle campagne
// lo produce l'agente adv nella sua cartella REPORT.
const TIPI = new Set(["copy", "grafica", "report-profili", "report", "shooting", "promo"]);
...
function dentroIConfini(percorso, cliente) {
  ...
  return resto.startsWith("output/") || resto.startsWith("contesto-generico/") || resto.startsWith("memoria/");
}

/** Il cliente deve esistere davvero: un nome sbagliato creerebbe un cliente fantasma. */
async function esiste(percorso) {
  try {
    const meta = await getMetadata(percorso);
    return Boolean(meta && meta.name);
  } catch (e) {
    // "non trovato" è una risposta, non un guasto: Dropbox però la lancia come
    // errore. Tutto il resto (rete, permessi) invece deve arrivare a chi chiama.
    if (/not_found/.test(String(e && e.message ? e.message : e))) return false;
    throw e;
  }
}
...
// ------------------------------------------------------------- memoria ----
// L'indice di cosa è già stato lavorato per un cliente. È l'unico file che si
// sovrascrive: non è un documento, è un registro che cresce. Viene riletto e
// riscritto intero ogni volta, quindi due persone che lavorano insieme sullo
// stesso cliente nello stesso minuto potrebbero pestarsi i piedi: con l'uso
// che se ne fa (un giro di copy alla volta) è un rischio accettabile.

const PERCORSO_INDICE = (cliente) => `${base()}/${cliente}/memoria/contenuti-fatti.json`;

export async function leggiIndice(cliente) {
  try {
    const testo = await downloadText(PERCORSO_INDICE(clientePulito(cliente)));
    const dati = JSON.parse(testo || "{}");
    return Array.isArray(dati.contenuti) ? dati : { contenuti: [] };
  } catch {
    return { contenuti: [] }; // mai lavorato: non è un errore
  }
}

/** Aggiunge alla memoria i contenuti appena consegnati, senza doppioni. */
export async function aggiornaIndice(cliente, nuovi) {
  if (!scritturaAttiva()) throw new Error("scrittura su Dropbox disattivata su questo ambiente");
  const c = clientePulito(cliente);
  const indice = await leggiIndice(c);

  const chiave = (x) => `${x.cartella || ""}/${x.file}`.toLowerCase();
  const gia = new Set(indice.contenuti.map(chiave));
  for (const n of nuovi) if (!gia.has(chiave(n))) { indice.contenuti.push(n); gia.add(chiave(n)); }
  indice.aggiornato = new Date().toISOString();

  const percorso = PERCORSO_INDICE(c);
  if (!dentroIConfini(percorso, c)) throw new Error("percorso fuori dai confini consentiti");
  await uploadFile(percorso, JSON.stringify(indice, null, 2), { modo: "overwrite" });
  return indice.contenuti.length;
}
