...
/**
 * I percorsi viaggiano dentro l'intestazione HTTP "Dropbox-API-Arg", che accetta
 * solo caratteri ASCII: un cliente come "Caffè Kirkas" o "Gennarì" la
 * farebbe fallire. Dropbox prevede proprio di scrivere gli accenti come \uXXXX.
 */
function apiArg(obj) {
...
/**
 * Scarica un file così com'è (byte grezzi). Serve per i loghi PNG o SVG:
 * l'anteprima di Dropbox li appiattirebbe su fondo bianco, perdendo la trasparenza.
 */
export async function downloadBinary(path) {
  const res = await call(
    `${CONTENT}/files/download`,
    { method: "POST", headers: { "Dropbox-API-Arg": apiArg({ path }) } },
    "download"
  );
  if (!res) return null;
...
/**
 * Carica un file. `modo` = "add" (default) NON sovrascrive mai: se il nome
 * esiste già Dropbox ne salva uno accanto rinominato. "overwrite" si usa solo
 * dove abbiamo già messo al sicuro la versione precedente.
 * Le cartelle mancanti le crea Dropbox da solo.
 */
export async function uploadFile(path, contenuto, { modo = "add" } = {}) {
  const dati = Buffer.isBuffer(contenuto) ? contenuto : Buffer.from(String(contenuto), "utf8");
  const res = await call(
  ...
  if (!res) return null;
  const json = await res.json();
  return { path: json.path_display || json.path_lower, nome: json.name, byte: json.size };
}

/** Copia un file (serve a mettere da parte la versione precedente). */
export async function copyFile(da, a) {
  const res = await call(
    `${RPC}/files/copy_v2`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ from_path: da, to_path: a, autorename: true }),
    },
    "copy"
  );
  if (!res) return null;
  const json = await res.json();
  return json.metadata ? json.metadata.path_display : null;
}

/** Link condivisibile a un file già caricato (per mandarlo al cliente). */
export async function linkCondivisibile(path) {
  try {
    const res = await call(
    ...
    // Se il link esiste già Dropbox protesta: si recupera quello esistente.
    if (/shared_link_already_exists/.test(String(e.message || e))) {
      const res = await call(
        `${RPC}/sharing/list_shared_links`,
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path, direct_only: true }) },
        "list_shared_links"
      );
      if (!res) return null;
 ...
 */
export async function getThumbnailBuffer(path, { size = "w1024h768", formato = "jpeg" } = {}) {
  const arg = apiArg({
...
export async function getThumbnailBase64(path, size = "w1024h768") {
  const buf = await getThumbnailBuffer(path, { size });
