...
/**
 * I copioni che l'app ha scritto per questo cliente, dal più recente.
 * Servono al Copy: i video sono girati DA questi testi, quindi sapere cosa
 * viene detto non richiede di trascrivere nulla — l'abbiamo scritto noi.
 * Ritorna [{ mese, nome, path }].
 */
export async function copioniArchiviati(cliente, quanti = 2) {
  const radice = `${base()}/${cliente}/output/shooting`;
  try {
    const mesi = (await listFolder(radice)) || [];
    const cartelle = mesi
      .filter((e) => e[".tag"] === "folder")
      .sort((a, b) => b.name.localeCompare(a.name)); // 2026-09 prima di 2026-08
    const trovati = [];
    for (const m of cartelle) {
      const file = (await listFolder(`${radice}/${m.name}`)) || [];
      // solo la versione testo: dal PDF l'app non saprebbe rileggere niente
      for (const f of file.filter((e) => e[".tag"] === "file" && /\.md$/i.test(e.name))) {
        const testoCopione = await testo(`${radice}/${m.name}/${f.name}`);
        if (testoCopione) trovati.push({ mese: m.name, nome: f.name, testo: testoCopione });
        if (trovati.length >= quanti) return trovati;
      }
    }
    return trovati;
  } catch {
