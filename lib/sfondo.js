...
export const TOLLERANZE = { leggera: 26, media: 46, forte: 78 };
 ...
 * Ritorna quanto è stato rimosso, per poter avvisare se non ha trovato nulla.
 */
export function rimuoviSfondo(img, tolleranza = TOLLERANZE.media) {
  const { data: d, width: w, height: h } = img;
  ...
  while (coda.length) {
    ...
    if (dd <= tolleranza) { d[p + 3] = 0; rimossi++; }
