...
2-3 frasi su cosa proponiamo per il periodo che viene.` : ""}`;
  ...
  const res = await fetch(API, {
    ...
    body: JSON.stringify({
      model: MODEL,
      // stesso motivo della didascalia: meglio largo che una risposta mozzata
      max_tokens: 2000,
      system: `${system}
...
- SCRIPT: il parlato, in PRIMA PERSONA DEL CLIENTE (non dell'agenzia).
  ...
  Lo script PROSEGUE dal gancio: non ripetere la frase del gancio all'inizio.
...
export async function scriviCopioni({ clientName, fascicolo, dati, repertorio, quanti = 6, singolo = null }) {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error("ANTHROPIC_API_KEY mancante");
...
/**
 * Didascalia di un contenuto, con il REGISTRO dichiarato.
 *
 * L'idea, dal confronto con William: se sappiamo cos'è andiamo nel dettaglio
 * ("banana split con panna"); se non lo sappiamo parliamo del momento senza
 * inventare il nome. Il registro esce insieme al testo, così chi rilegge sa in
 * due secondi se fidarsi — e si vede quali cartelle vanno arricchite.
 *
 * Regola dura: SPECIFICO solo se DUE fonti indipendenti concordano, cioè un
 * nome (file, cartella o commento dell'operatore) e ciò che si vede davvero.
 */
export async function scriviDidascalia({ clientName, fascicolo, images, kind = "foto", indizi = {} }) {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error("ANTHROPIC_API_KEY mancante");
  const shots = (images || []).filter(Boolean);
  if (!shots.length) throw new Error("nessuna immagine da guardare");
  ...
  const f = fascicolo || {};
  const pezzo = (t, x) => (x ? `\n### ${t}\n${String(x).slice(0, 3500)}` : "");

  const system = `Sei il social media manager di MiglioriAmo che scrive per un'attività locale. ${CRAFT}

I DUE REGISTRI - è la regola più importante:
- SPECIFICO: usalo SOLO se sai davvero cos'è. Serve che due fonti indipendenti
  concordino: un nome (del file, della cartella o il commento dell'operatore) E
  quello che vedi nell'immagine. Allora nomina il prodotto e i dettagli.
- CATEGORIA: vedi di che genere di cosa si tratta (un gelato, una pizza, un
  taglio di capelli) ma non quale esattamente. Parla della categoria e di ciò
  che si vede, senza inventare nomi o ingredienti.
- GENERICO: non si capisce cosa sia. Parla del momento, del gesto, dell'occasione.
  Resta concreto: niente frasi da agenzia tipo "un'esperienza sensoriale unica".

Se il nome del file dice una cosa e l'immagine ne mostra un'altra, NON fidarti
del nome: scendi di registro e dillo nel motivo.

Se ci sono ISTRUZIONI DEL PROJECT MANAGER, non sono un suggerimento: sono il
brief. Contengono spesso prodotti, prezzi, marchi, ordine delle slide, persone
da taggare o la frase esatta da usare — rispettali alla lettera. Valgono come
fonte attendibile, perché le ha scritte una persona che era sul posto.

Se c'è la TRASCRIZIONE del parlato, quella è la fonte migliore: dice cosa il
cliente racconta davvero nel video. Vale come una delle due fonti concordanti,
e i fotogrammi servono a confermare il contesto. Non copiare il parlato parola
per parola: la didascalia accompagna il video, non lo ripete.

NON INVENTARE MAI nomi, ingredienti, prezzi, orari, indirizzi che non siano nel
fascicolo qui sotto. Usa le CTA pronte quando ci sono. Rispetta i paletti.`;

  const contesto =
    pezzo("Chi è il cliente", f.dossier) +
    pezzo("Dati operativi (gli unici numeri che puoi usare)", f.dati) +
    pezzo("CTA pronte", f.cta) +
    pezzo("PALETTI - cosa non dire", f.paletti);

  const fonti = [
    indizi.istruzioni
      ? `ISTRUZIONI DEL PROJECT MANAGER (le ha scritte nel nome del file o della cartella, vanno SEGUITE):\n"${String(indizi.istruzioni).slice(0, 900)}"`
      : null,
    indizi.parlato
      ? `TRASCRIZIONE DEL PARLATO nel video (è la fonte più affidabile: è ciò che il cliente dice davvero):\n"${String(indizi.parlato).slice(0, 12000)}"`
      : null,
    indizi.commento ? `Commento dell'operatore su questo gruppo di contenuti: "${indizi.commento}"` : null,
    indizi.copioni
      ? `COPIONI SCRITTI PER QUESTO CLIENTE (i video sono girati da questi testi: se riconosci la scena, sai già cosa viene detto):\n${String(indizi.copioni).slice(0, 4000)}`
      : null,
    indizi.nomeFile
      ? `Nome del file: "${indizi.nomeFile}"`
      : "Il nome del file non dice niente (è un codice di macchina fotografica).",
    indizi.nomeCartella ? `Nome della cartella: "${indizi.nomeCartella}"` : null,
  ].filter(Boolean).join("\n");

  const istruzioni =
    kind === "video"
      ? `Queste sono ${shots.length} inquadrature prese in ordine dallo STESSO video: ricostruisci cosa mostra la clip nel suo insieme.`
      : "Guarda questa immagine.";

  const richiesta = `${istruzioni}

FONTI DI INFORMAZIONE OLTRE ALL'IMMAGINE
${fonti}

FASCICOLO DI "${clientName}"${contesto || "\n(vuoto: resta sul generico)"}

Rispondi ESATTAMENTE così, senza premesse:

[DIDASCALIA]
<2-3 frasi + CTA + 3-5 hashtag>
[REGISTRO]
<specifico | categoria | generico>
[PERCHE]
<una riga: su cosa ti sei basato, o cosa mancava>`;
  ...
  const res = await fetch(API, {
    ...
    body: JSON.stringify({
      model: MODEL,
      // 700 non bastavano: la richiesta porta con sé tutto il fascicolo e il
      // modello finiva lo spazio PRIMA di scrivere la didascalia (stop_reason
      // max_tokens, risposta vuota).
      max_tokens: 2000,
      system,
      messages: [{
        role: "user",
        content: [
          ...shots.map((data) => ({ type: "image", source: { type: "base64", media_type: "image/jpeg", data } })),
  ...
  if (!testo)
    throw new Error(
      json.stop_reason === "max_tokens"
        ? "il modello ha esaurito lo spazio prima di scrivere: riprova"
        : "il modello non ha risposto niente"
    );
  ...
  const registro = (sez("REGISTRO") || "").toLowerCase().replace(/[^a-z]/g, "");
  const didascalia = sez("DIDASCALIA") || testo.replace(/\[[A-Z]+\]/g, "").trim();
  return {
    caption: didascalia,
    registro: ["specifico", "categoria", "generico"].includes(registro) ? registro : "generico",
    motivo: sez("PERCHE"),
    // Se la lettura fallisce si porta dietro la risposta grezza: senza, un
    // riquadro vuoto non dice a nessuno cosa sia andato storto.
    grezzo: didascalia ? undefined : testo.slice(0, 500),
    fine: json.stop_reason,
  };
}
