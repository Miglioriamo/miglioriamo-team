...
${adv
  ? "- Questo cliente INVESTE in advertising: puoi parlarne, ma solo in relazione a ciò che è negli appunti."
  : "- Questo cliente NON investe in advertising: non nominare MAI campagne, sponsorizzate, budget o advertising in nessuna forma."}`;
...
{"racconto": "4-6 frasi che raccontano cosa abbiamo fatto nel periodo, in un discorso unico e scorrevole"${conProposta ? ', "proposta": "2-3 frasi su cosa proponiamo per il periodo che viene"' : ""}}`;
...
${(dossier || "(nessun dossier disponibile — resta prudente e generico)").slice(0, 5000)}`,
  ...
  // Claude a volte incornicia il JSON: si prende comunque il primo oggetto.
  const grezzo = testo.slice(testo.indexOf("{"), testo.lastIndexOf("}") + 1);
  try {
    const out = JSON.parse(grezzo);
    return { racconto: (out.racconto || "").trim(), proposta: (out.proposta || "").trim() };
  } catch {
    return { racconto: testo, proposta: "" };
  }
}
